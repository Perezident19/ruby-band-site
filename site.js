...
function renderAdditionalPlayerCard(member) {
  ...
  const instagramLink = getInstagramLink(member.instagram_url);
  const instagramLabel = String(member.instagram_url || "").trim();
  const memberClass = name.toLowerCase() === "ani" ? " member-card-ani" : "";

  return `
    <article class="member-card additional-player-card${memberClass}">
      <div class="member-photo">
        ${
          photoUrl
            ? `<img src="${escapeHtml(photoUrl)}" alt="${escapeHtml(name)} of Ruby" loading="lazy" referrerpolicy="no-referrer" />`
...
function renderMembers(members) {
  ...
  if (additionalPlayersList && additionalPlayersSection) {
    additionalPlayersList.innerHTML = additionalPlayers.map(renderAdditionalPlayerCard).join("");
...
function renderEventCard(event) {
  ...
  const action = ticketUrl
    ? `<a class="button button-secondary" href="${escapeHtml(ticketUrl)}">Buy Tix</a>`
    : "";
